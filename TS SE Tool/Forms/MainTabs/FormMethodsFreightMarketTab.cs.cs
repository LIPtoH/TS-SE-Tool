    ...
    public partial class FormMain
    {
        ...
        internal Image freightMarketGetCompanyIcon(string _companyName, Brush _brush)
        {
            if (File.Exists(@"img\" + GameType + @"\companies\" + _companyName + ".dds"))
                return Utilities.Graphics.ddsImgLoader(new string[] { @"img\" + GameType + @"\companies\" + _companyName + ".dds" }, 100, 32, 0, 0)[0];
            else
            {
                ...
                if (files.Length > 0)
                    return Utilities.Graphics.ddsImgLoader(new string[] { files[0] }, 100, 32, 0, 0)[0];
                else                
